...
export type Room = { id: string; title: string; status: string; mode: VotingMode; winner_id: string | null; created_at: string; revision: number; isHost: boolean; candidates: Restaurant[]; votes: Vote[]; myVote: Vote | null; total: number; orders: FoodOrder[] };
export type Catalog = { restaurants: Restaurant[]; rooms: { id: string; title: string; status: string; mode: VotingMode }[] };
export type HistoryRoom = { id: string; title: string; status: string; mode: VotingMode; created_at: string; deleted_at: string | null; winner_name: string | null; vote_count: number; order_count: number };
