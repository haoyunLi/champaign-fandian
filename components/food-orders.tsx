...
export function FoodOrders({ room, update }: { room: Room; update: (r: Room) => void }) {
  ...
  return <section className="food-orders" id="food-orders"><div className="section-heading"><div><h2><ShoppingBag />带饭清单</h2><p>{winner?.name} · {room.orders.length} 条登记 / {count} 份{pending ? ` · ${pending} 条待认领` : ''}</p></div><Button className="primary order-cta" onClick={() => { orderId.current = crypto.randomUUID(); setError(''); setAdding(true); }}><Plus />帮我带一份</Button></div>
    <p className="order-explainer">今天去不了？写下想吃的菜，去的朋友可以认领。这里只登记带饭，菜品是否有售、价格和付款请在群里确认。</p>
    {!room.orders.length ? <div className="orders-empty"><HandHeart /><div><strong>人不在，也能一起开饭。</strong><p>还没有带饭登记。填好菜名和备注，等朋友认领吧。</p></div></div> : <div className="orders-list">{room.orders.map(o => <article className={`order-row order-${o.status}`} key={o.id}><div className="order-main"><div className="order-title"><strong>{o.dish}</strong><span>× {o.quantity}</span></div><p><b>{o.nickname}</b> 想吃{o.isMine && <span className="mine-label">我的登记</span>}</p>{o.note && <p className="order-note">{o.note}</p>}</div><div className="order-actions"><span className={`order-status ${o.status}`}>{o.status === 'pending' ? '等人带饭' : o.status === 'claimed' ? `${o.claimant_name} 来带` : `${o.claimant_name} 已带回`}</span><div className="order-buttons">{o.status === 'pending' && <Button className="secondary" variant="outline" disabled={busy} onClick={() => { setError(''); setClaim(o); }}><HandHeart />我来带</Button>}{o.status === 'pending' && o.isMine && <Button size="icon" variant="ghost" aria-label={`取消 ${o.dish}`} disabled={busy} onClick={() => { setError(''); setCancel(o); }}><X /></Button>}{o.status === 'claimed' && o.canManage && <><Button className="secondary" variant="outline" disabled={busy} onClick={() => act({ action: 'deliver', orderId: o.id })}><Check />已带回</Button><Button size="icon" variant="ghost" aria-label={`取消认领 ${o.dish}`} disabled={busy} onClick={() => act({ action: 'release', orderId: o.id })}><Undo2 /></Button></>}</div></div></article>)}</div>}
