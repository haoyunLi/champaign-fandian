  ...
  return <section className="food-orders" id="food-orders">
    ...
    {!visible.length ? <div className="orders-empty"><HandHeart/><div><strong>{emptyText[filter]}</strong><p>{filter==='carrying'?'在「待认领」里选择你可以帮忙带的菜。':filter==='mine'&&!finished?(stopped?'本轮已停止接收新登记。':'点击「帮我带一份」填写菜名和备注。'):filter==='pending'&&room.orders.length?'可以切换到「全部」查看认领和带回情况。':finished?'投票结果仍可查看。':stopped?'本轮已停止接收新登记。':'去不了的朋友可以登记，去的朋友可以认领。'}</p></div></div> : <div className="orders-list">{visible.map(o=><article className={`order-row order-${o.status}`} key={o.id}>
      ...
      <div className="order-actions"><span className={`order-status ${o.status}`}>{orderStatus(o)}{finished&&o.status!=='delivered'?'（本轮已结束）':''}</span>
        {!finished&&<div className="order-buttons">{o.status==='pending'&&<Button className="secondary" variant="outline" disabled={busy} onClick={()=>{setError('');if(!carrier)setCarrier(room.preferred_nickname||room.myVote?.nickname||'');setClaim(o);}}><HandHeart/>我来带</Button>}{o.status==='pending'&&o.isMine&&<><Button variant="ghost" disabled={busy} aria-label={`编辑 ${o.dish}`} onClick={()=>openEditor(o)}><Pencil/>编辑</Button><Button size="icon" variant="ghost" aria-label={`取消 ${o.dish}`} disabled={busy} onClick={()=>{setError('');setCancel(o);}}><X/></Button></>}{o.status==='claimed'&&o.isMine&&<Button variant="ghost" disabled={busy} onClick={()=>openEditor(o)}>{o.change_request?'修改改单申请':'申请换菜 / 改单'}</Button>}{o.status==='claimed'&&o.canManage&&<><Button className="secondary" variant="outline" disabled={busy||o.purchase_status==='sold_out'||!!o.change_request} onClick={()=>{setError('');if(unfinished.length===1)setDelivery(o);else void act({action:'deliver',orderId:o.id,expectedRevision:o.revision});}}><Check/>已带回</Button><Button size="icon" variant="ghost" aria-label={`取消认领 ${o.dish}`} disabled={busy||o.purchase_status==='ordered'||!!o.change_request} onClick={()=>void act({action:'release',orderId:o.id,expectedRevision:o.revision})}><Undo2/></Button></>}</div>}
      </div>
      {(!finished||o.issue_note||o.change_request)&&<OrderPurchase order={finished?{...o,isCarrier:false}:o} busy={busy} error={error} act={act}/>}
    </article>)}</div>}
    ...
    <Dialog open={!!editor} onOpenChange={open=>!open&&!busy&&setEditor(null)}><DialogContent className="editor-dialog order-composer"><DialogHeader><DialogTitle>{editor==='new'?'我不去，帮我带饭':editor?.status==='claimed'?'申请换菜 / 改单':'修改我的带饭'}</DialogTitle><DialogDescription>{winner?.name} · {editor==='new'?'填写具体菜名，方便朋友下单。':editor?.status==='claimed'?'带饭人确认后才替换原登记；请先与对方联系。':'保存后保留这条登记，无需取消重填。'}</DialogDescription></DialogHeader>
      <div className="order-composer-grid">{winner&&<aside className="composer-menu"><h3>对照菜单填写</h3><MenuBrowser key={editor==='new'?'new':editor?.id} restaurant={winner} roomId={room.id} initialMode={menuMode}/></aside>}<form onSubmit={async e=>{
        ...
        {error&&<p className="error" role="alert">{error}</p>}<Button className="primary full" disabled={busy||finished||editChanged||(editor==='new'&&stopped)}>{busy?<Loader2 className="spin"/>:<Check/>}{busy?'正在保存…':editor==='new'?pendingOrder?'重试，找回上次登记':'加入带饭清单':editor?.status==='claimed'?'提交申请，等待带饭人确认':'保存修改'}</Button>
