...
async function catalog(db: D1Database, owner: string) {
  ...
  return { pools,profile:await profile(db,owner),restaurants:(restaurants.results as RestaurantRow[]).map(restaurantWithMedia),
