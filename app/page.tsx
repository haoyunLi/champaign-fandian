...
function RoomView({ room, setRoom, home }: { room: Room; setRoom: (r: Room) => void; home: () => void }) {
  ...
  return <>
    ...
    <section className="room-heading"><div><h1>{room.title}</h1><p>{phase==='finished' ? '本轮饭局已结束，结果和带饭清单仍可查看。' : closed ? '餐馆已确定，继续登记和安排带饭吧。' : manual ? '每人选一家想吃的餐馆，票数最多的就是目的地。' : '每人随机抽一家，让大家的运气一起决定。'}</p></div><div className="room-badges"><span className="mode-badge">{manual ? '自主投票' : '随机抽签'}</span><MealStatus meal={room} /></div></section>
    <div className="room-grid"><section className={`draw-panel ${closed ? 'winner-panel' : ''}`} aria-live="polite">
      ...
      : mine ? <><div className="draw-number">{manual ? <Check /> : String(mine.position).padStart(2, '0')}</div><p className="draw-label">{manual ? '你已投票给' : '你的随机号码，已自动投票给'}</p><h2>{mine.name}</h2><p className="voted"><Check size={18} />{room.myVote?.nickname}，这一票已计入</p><p className="muted">等群友到齐，由发起人确定餐馆。</p></>
    ...
    <section className="results"><div className="section-heading"><h2>{closed ? '最终票数' : '实时票数'}</h2><span className="count"><Users size={17} />{room.total} 人已投</span></div>
      ...
      {room.isHost && !closed && <div className="host-actions"><Button variant="outline" className="secondary full" disabled={!room.total || busy} onClick={() => setConfirm(true)}>结束投票，确定餐馆</Button><p className="fine-print">仅你作为发起人可以结束；平票时随机选一家。</p></div>}
      {!room.isHost && !closed && <p className="host-note">大家投完后，由发起人确定餐馆，再进入带饭阶段。</p>}
    </section></div>
    ...
    <Dialog open={confirm} onOpenChange={setConfirm}><DialogContent className="editor-dialog"><DialogHeader><DialogTitle>大家都投完了吗？</DialogTitle><DialogDescription>确定后不再接收新票。最高票餐馆获胜，平票随机选一家。随后进入带饭阶段：全部带回，或确定餐馆满 12 小时后，饭局才结束。</DialogDescription></DialogHeader><Button className="primary full" disabled={busy} onClick={async () => { setBusy(true); setError(''); try { setRoom(await api<Room>({ action: 'close', room: room.id })); setConfirm(false); } catch (e) { setError((e as Error).message); setConfirm(false); } finally { setBusy(false); } }}>{busy ? '正在决定…' : '确定餐馆，进入带饭'}</Button></DialogContent></Dialog>
  </>;
}
