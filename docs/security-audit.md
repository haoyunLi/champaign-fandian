...
旧版本锁定了 React Server DOM、Vite、Vinext 及 Cloudflare 工具链的已知告警。部分问题仅影响开发服务器或构建时解析，不能将全部依赖告警等同于在线可利用漏洞；首次审查时应用没有 Server Actions 或用户图片上传入口。后来加入的菜单上传审查记录见下文。
...
## 登录与昵称保留补充审查 · 2026-09-12

新增可选的 Sites / ChatGPT 账号登录。账号只从可信平台认证头解析，其他托管域名默认禁用该认证路径。原游客数据仅在账号首次建立时关联，唯一约束防止一个游客身份归属两个账号。退出后旧游客令牌不能取回已绑定账号的管理权限；写入核对当前界面身份，跨账号旧表单返回冲突。

34 项自动测试通过，新增测试覆盖实际签发 Cookie 的昵称保留、首次关联、跨浏览器找回、退出与重新登录、不同账号隔离、并发首次登录、陈旧页面写入、跨站请求以及非可信域名的伪造认证头。手机预览使用平台自带的本地模拟登录验证入口、昵称延续、退出和再次登录；真实第三方账号登录由平台托管，未代替用户提交真实账号授权。

## 2026-09-12 官网与菜单增量审查

- 官网只接受 HTTP(S)，拒绝脚本、数据链接及带用户名密码的 URL；服务器不抓取用户填写的网址。
- 菜单仅接收有界栅格图片，流式读取限制 5 MB，不信任 Content-Length；拒绝 SVG/HTML、MIME 不一致及无效容器。图片路由固定类型、nosniff 和隔离 CSP。容器识别不等同于完整图片解码，浏览器无法显示时提供重新加载。
- 图片上传沿用身份变化与跨站写入防护，未发布图片只能本人预览及关联；图片就绪核验与关联公开在同一数据库事务内完成。随机编号不能充当身份凭据。
- 重试同一请求不会重复写对象，改变重试内容会被拒绝；存储失败可恢复。过期未保存图片清理不影响已发布及历史引用。
- 真实 API/SQLite/R2 接口替身新增 7 项流程测试，总计 41 项通过；包含菜单快照、旧客户端兼容、错误格式、大小限制、重试、身份和上传限额。另检查实际本地 Worker 的上传、读取及手机界面。


## 2026-09-13 · 六项使用流程改进复审

新增餐馆池、最新菜单、取餐安排及菜单内登记。检查个人数据隔离、旧餐馆快照、认领权限、过期状态、多页面版本冲突、草稿与正式提交边界。50 项自动测试覆盖旧功能及新行为，其中新增 9 项，原菜单和登录测试扩展覆盖上传后读取新菜单、账号绑定后找回餐馆池。数据库升级检查使用包含旧餐馆和饭局的数据库，确认新增迁移保留记录与外键一致性。

取餐安排仅当前带饭人可写，参数不能指定另一位所有者；服务器在写入时再次核对生命周期和当前认领。餐馆池只向本人查询，创建重试不重复新增；旧窗口不能覆盖较新内容。最新菜单始终从本轮候选关联餐馆读取，旧记录存在歧义时不猜测。群通知是只读整理及浏览器复制，不自动向外发送。未提交草稿仅暂存在当前标签页，更换身份时清除。

本次没有浏览器自动化或微信真机测试；界面通过类型、代码检查及生产构建验证，服务行为另在隔离的真实 Worker/D1 运行环境检查。


## 2026-09-13 · 下单流程、个人更新与截止复审

新增 13 项 API／纯函数／迁移测试，总计 63 项。覆盖当前认领人权限、下单后退回限制、售罄禁止带回、改单前保留原菜品、接受时餐馆确认、旧申请版本冲突、改单与带回并发、通知所有者隔离、并发未读保留、个人菜品按餐馆隔离、确切备注汇总、创建截止重试、自动结果幂等、无人投票结果、数据库执行时跨截止的投票拒绝、香槟夏令时和已有数据升级。

检查中保留原先人人可结束投票的业务规则。购买状态不代替向餐馆下单；已下单后的实际调整需由当前带饭人确认。全部 SQL 写入条件包含身份、版本与有效生命周期，申请内容参数化存储，前端作为文本展示。通知与上次菜品只读取当前身份的数据。

未新增依赖或外部消息通道。此轮使用自动测试、类型与代码检查、构建及隔离 Worker/D1 服务检查；未进行浏览器自动化或微信真机测试。
